...
from digash_v3_events import detect_events, dedup_events, assign_targets, simulate
...
def main() -> int:
    ...
    log(f"=== DIGASH REPLICATION V3 | {pair} ===")
    progress("load", 0, 1)
    ...
    for i, e in enumerate(events):
        ...
        row = simulate(x5, e, pair, targets.get(i, np.nan))
        if row is None:
            continue
    ...
    log(f"DONE|{pair}|levels={len(levels)}|raw_events={raw_n}|events={len(df)}|elapsed={time.monotonic()-t0:.1f}s")
    return 0
