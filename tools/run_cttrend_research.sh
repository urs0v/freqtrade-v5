...
echo "Skip backfill: $SKIP_BACKFILL"
echo
...
if [[ "$SKIP_BACKFILL" != "1" ]]; then
  ...
  echo "[1/2] Backfill skipped; reusing existing SQLite archive."
fi

echo
echo "[2/2] Running pre-registered unbiased CTREND V3 reconstruction..."
python /opt/rmv5/tools/cttrend_research_v3.py \
  --db "$DB" \
  --start "$EVAL_START" \
  --end "$EVAL_END" \
  --universe 50 \
