...
OUTDIR="${OUTDIR:-/freqtrade/user_data/digash_fidelity_v41}"
WORKERS="${WORKERS:-16}"
WARMUP_DAYS="${WARMUP_DAYS:-120}"
BAND_PCT="${BAND_PCT:-0.10}"
mkdir -p "$OUTDIR"
echo "=== DIGASH FIDELITY V4.1 RUNNER ==="
echo "NULL-CONTROL + ALERT TIMING: no PnL, no detector tuning, no downloads."
echo "WORKERS=$WORKERS WARMUP_DAYS=$WARMUP_DAYS BAND_PCT=$BAND_PCT"
echo "V4DIR=$V4DIR OUTDIR=$OUTDIR"
PYTHONUNBUFFERED=1 python -u /opt/rmv5/tools/audit_digash_fidelity_v41.py \
  --config "$CONFIG" \
  --datadir "$DATADIR" \
  --v4dir "$V4DIR" \
  --outdir "$OUTDIR" \
