...
def parse_args():
    p = argparse.ArgumentParser(description="Parallel Digash replication V3 runner")
    ...
    p.add_argument("--outdir", default="/freqtrade/user_data/digash_replication_v3")
...
def main() -> int:
    ...
    print("=== DIGASH REPLICATION V3 ===", flush=True)
    print(f"pairs={len(pairs)} | workers={workers} processes | CACHE ONLY", flush=True)
    print("Public-core test: multi-TF horizontal levels (period 20/30, 1 counted touch), active coins, breakout/retest/bounce/fakeout.", flush=True)
    print("No historical trade-count, densities, or invented closed trend-line algorithm.", flush=True)

    print("\nPreparing Digash-style activity lists...", flush=True)
    alog = out / "activity.log"
    cmd = [
        sys.executable, "-u", "/opt/rmv5/tools/prepare_digash_activity_v3.py",
    ...
    with alog.open("w", encoding="utf-8") as lf:
        ...
        for raw in pr.stdout:
            ...
            if line.startswith("ACTIVITY|"):
                ...
                try:
                    i, n = int(f[1]), int(f[2])
                    sys.stdout.write(f"\r{bar(i/n)} activity {i}/{n}")
                    sys.stdout.flush()
                except Exception:
                    pass
        rc = pr.wait()
    ...
    states = {
        p: {"phase": "load", "frac": 0.0, "done": False, "failed": False, "started": None}
        for p in pairs
    }
    lock = threading.Lock()
    ...
    def dashboard():
        while not stop.is_set():
            ...
            sys.stdout.write(
                ...
                f"| failed {fail} | elapsed {fmt(el)} | ETA {fmt(eta)} | {pt}"
            )
            sys.stdout.flush()
            stop.wait(1)
    ...
    def run_pair(pair):
        ...
        cc.setdefault("exchange", {})["pair_whitelist"] = [pair]
        cp.write_text(json.dumps(cc, indent=2))
        log = part / "worker.log"
        af = manifest.get(pair)
        if not af:
            return pair, 98, log
        cmd = [
            sys.executable, "-u", "/opt/rmv5/tools/audit_digash_replication_v3.py",
                ...
                if line.startswith("PROGRESS|"):
                    ...
                    if len(f) >= 4:
                        try:
                            ...
                            ph = f[1]
                            with lock:
                                states[pair]["phase"] = ph
                                states[pair]["frac"] = min(1, d / tot)
                        except ValueError:
                            pass
            rc = pr.wait()
        with lock:
    ...
    if fail:
        ...
        return 2
    print("Aggregating replication study...", flush=True)
    return subprocess.call([
        sys.executable, "-u", "/opt/rmv5/tools/aggregate_digash_replication_v3.py",
        "--parts", str(parts), "--outdir", str(out),
    ])
